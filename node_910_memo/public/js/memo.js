document.addEventListener("DOMContentLoaded", () => {
  ...
  toImage.addEventListener("change", (event) => {
    ...
    // URL.createObjectURL() 또는 webKitURL.createObjectURL() 함수 중에 하나 사용하여
    // 업로드한 이미지를 가상의 이미지(객체)로 변환하여 화면에 미리보기를 구현하라
    // 이 함수는 CSP 정책에 따라 새용이 제한된다
  ...
  이때, 한 개의 메모를 구성하는 요소들은 li, img, span tag 로 되어 있다.
  어떤 부분을 클릭하였을때, 메모의 seq 를 알려줄지 만들기 위해서 
  ...
  3. memo 가 포함된 target 을 알게 되면
    ...
    3-2. 만약 classList 에 lsit 가 포함되어 있다면 
        이 target 은 LI tag 이다.
    3-3. LI tag 에는 data-seq 속성이 설정되어 있으므로 
        자신이 dataset,seq 값을 getter 하면 된다.
    3-4. classList 에 list 가 포함되어 있지 않다면, 
        target 은 img 이거나 span 이다.
    3-5. target.closest("LI") 를 실행하여 
        부모 LI tag 로 부터 dataset.seq 를 getter 한다.
  */
  memo_box.addEventListener("click", async (e) => {
    ...
    if (classList.contains("memo")) {
      ...
      // 서버에서 받은 메모데이터를 각각의 input tag 의 value 값에 setting 하여 
      // 수정데이터가 보이도록 하기
